...
import (
	...
	"github.com/truekupo/cluster/solana-stake-srv/server"
...
func NewService(conf *config.Config, stderr bool) execute.Service {
	...
	// Init logger
	logger.InitLogger(conf.System.LogLevel, conf.System.LogFormat, conf.System.LogFile, stderr, map[string]interface{}{"srv": s.Name(), "chain": conf.Symbol})
...
func (s *service) Init() error {
	...
	lh, err := stake.NewSolanaStakeHandler(s.cfg)
	if err != nil {
		log.WithField("error", err).Error("init")
		return err
	}

	gs, err := server.NewGrpcServer(s.cfg, lh)
	if err != nil {
